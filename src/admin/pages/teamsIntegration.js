...
const TeamsIntegration = (props) => {
    ...
    const consetVerified = props.location.search.includes('admin_consent=True') && props.location.search.includes(`tenant=${client.config.tenantId}`) || false
                ...
                <Row className='steps-container'>
                    <Col span={24}>
                        <Card>
                            ...
                            {current === 1 && <GetConsent client={client} setCurrent={setCurrent} state={state} />}
...
const GetConsent = ({ client, state ,setCurrent}) => {
    ...
    const redirectToConsent = () => {
        Modal.confirm({
            ...
            cancelText: 'No',
            onOk() {
                window.location.href = `https://login.microsoftonline.com/${client.config.tenantId || tenantId}/adminConsent?client_id=35a2ff7a-28a5-466d-9f84-1cefbe80c187&redirect_uri=https://localhost:3000/teams-integration/`;
            },
            onCancel() {
                console.log('Cancel');
            }
    ...
    return (
        <div className='login-with-ms-widget' >
            <Row>
                <Col span={24}>
                    <Card bordered={false} actions={[
                        showNext && <Button size='large' icon={<RightOutlined />} type='primary' onClick={() => setCurrent(2)} >Next</Button>,
                    ]} >
                        {!showNext ?
                            <div>
