...
import { Breadcrumb, Button, Card, Col, Layout, ConfigProvider, Form, Row, Typography, Switch, Slider, Input, Select, theme, Space, Alert, Menu, Tabs, Modal, notification, Table, Spin } from 'antd';
...
const UsersManagement = () => {
    ...
    const getlistGroupsForUser = async (username, email, name) => {
        ...
        API.get(apiName, path, myInit)
            .then((response) => {
                ...
                var role = "";
                if (response.Groups[0].GroupName === 'p3fAdmin')
                    role = 'Admin';
    ...
    const removeUser = async (selectedRow, check) => {
        ...
        if (selectedRow.role === 'Admin')
            role = 'p3fAdmin';
        else if (selectedRow.role === 'Support User')
            role = 'p3fSupport';
    ...
    return (
        <div className='content-container'>
            <div className='main-container'>
                ...
                <Modal title={<h2> Create User</h2>} className='CreateModal' open={createModalVisible} onOk={form.submit} onCancel={handleCreateCancel} >
                    <Form
                        form={form}
                        name="createForm"
                        labelCol={{
                ...
                <Modal title={<h2> Edit User</h2>} className='editModal' open={editModalVisible} onOk={formEdit.submit} onCancel={handleEditCancel} >
                    <Space
                        direction="vertical"
                        size="middle"
                        style={{
