...
export default function PresolvedAuditUpdateForm(props) {
  const {
    id: idProp,
    presolvedAudit: presolvedAuditModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = presolvedAuditRecord
      ? { ...initialValues, ...presolvedAuditRecord }
      : initialValues;
    setTenantId(cleanValues.tenantId);
    setResource(cleanValues.resource);
    setAction(cleanValues.action);
    setByUser(cleanValues.byUser);
    setByDateTime(cleanValues.byDateTime);
    setChangesMade(
      typeof cleanValues.changesMade === "string"
        ? cleanValues.changesMade
        : JSON.stringify(cleanValues.changesMade)
    );
    setErrors({});
  };
  const [presolvedAuditRecord, setPresolvedAuditRecord] = React.useState(
    presolvedAuditModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(PresolvedAudit, idProp)
        : presolvedAuditModelProp;
      setPresolvedAuditRecord(record);
    };
    queryData();
  }, [idProp, presolvedAuditModelProp]);
  React.useEffect(resetStateValues, [presolvedAuditRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            PresolvedAudit.copyOf(presolvedAuditRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PresolvedAuditUpdateForm")}
      {...rest}
    >
      <TextField
        label="Tenant id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || presolvedAuditModelProp)}
